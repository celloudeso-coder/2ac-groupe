...
interface Props {
  service: Pick<Service, 'slug' | 'title' | 'short_desc' | 'icon' | 'engagements' | 'brand_name' | 'accent_color' | 'cover_image'>
  compact?: boolean
}
...
export default function ServiceCard({ service, compact = false }: Props) {
  ...
  return (
    <Link
      href={`/services/${service.slug}`}
      className="group card-glass hover-glow flex h-full flex-col overflow-hidden"
    >
      {service.cover_image && (
